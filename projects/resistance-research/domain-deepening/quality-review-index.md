# Domain-Deepening Quality Review Index

*Completed April 2026. Full read of all 23 domain-deepening companion files.*

*Index updated April 2026, Session 265: five Tier 3 files elevated following targeted deepening in Sessions 263–265. Priority 1 items in the Deepening Queue are complete. See tier notes below.*

*Index updated April 2026, Session 267: judicial-independence-evidence.md elevated from Tier 2 to Tier 1. Three gaps filled: fiscal estimates for SCOTUS expansion, term limits, and independent ethics IG (Section 12); full development of Canada JAC/JAAC model and South Africa JSC (Section 13); counterargument on court reform as politicization with counter-counter and policy sequencing resolution (Section 14).*

*Index updated April 2026, Session 269: economic-concentration-evidence.md elevated from Tier 2 to Tier 1. Actionable intelligence gap filled: Section 10 added covering live enforcement proceedings (Google Search remedy phase, FTC v. Amazon, DOJ v. Apple, DOJ v. Google Ad Tech, FTC PBM investigation), named legislative vehicles (CALERA, AICOA, Open App Markets Act, Farm System Reform Act, state-level modernization), state AG enforcement actions and coalition patterns, organizations with pressure-point specificity (Open Markets Institute, American Economic Liberties Project, EFF, Demand Progress, state PIRGs), and three time-bounded 2026 windows (Google remedy record, FY2027 appropriations, state AG coalition expansion).*

*Index updated April 2026, Session 268: housing-evidence.md elevated from Tier 2 to Tier 1. Four gaps filled: Vienna Gemeindebau section expanded with construction cost comparisons (€150-200k/unit vs. US LIHTC $400-600k), rent levels (€7-8/sqm vs. €15-20/sqm market), waitlist data (26,000 households, ~1-2 year wait), income ceiling (€53,340/year net), Klimabonus passive-house mandate, and Gemeinschaftliche Wohnprojekte co-housing model; Singapore HDB section added as Section 9.1b with coverage (79% of residents), homeownership rate (88.9%), resale prices (SGD 500-600k vs. SGD 1.2M+ private), construction cost (SGD 200-350k/unit), BTO scheme, income ceiling (SGD 14,000/month), and Land Acquisition Act as irreplaceable enabling condition; fiscal estimates added for zoning reform (YIMBY Act near-zero cost, transportation conditioning $5-15B leverage, Hsieh-Moretti $1.7T GDP gain, Minneapolis 6-10% rent reduction), LIHTC alternatives (all-in $400-600k/unit current vs. $280-420k direct grant alternative, Vienna non-profit analog at $180-280k/unit), and CLT scaling ($2.5-4B over 10 years vs. $31-37.5B voucher equivalent); Actionable Intelligence section added as Section 13 (federal legislation vehicles, state pressure points through 2026, organizations to support, specific appropriations and rulemaking pressure points); 2025-2026 rollback documentation added as Section 14 (AFFH suspension affecting $7B in annual grants, HUD discretionary budget cut ~20%, 400 FTE reduction, Section 8 under-funded by 100,000 vouchers, manufactured housing code update as surviving regulatory gain, record 770,000 PIT count backdrop).*

*Index updated April 2026, Session 270: national-security-evidence.md elevated from Tier 2+ to Tier 1. Actionable intelligence gap filled: existing placeholder AI section replaced with comprehensive Sections AI-1 through AI-6 covering: (1) most immediately actionable items keyed to FY2027 NDAA markup window (May-August 2026); (2) legislative vehicles with bill numbers and procedural posture — H.R. 6751 (Jayapal-Massie 2001 AUMF sunset, 240-day repeal), H.R. 7555 (Audit the Pentagon Act, Pocan-Biggs, 0.5%/1% penalty structure), enacted 2002/1991 AUMF repeal via FY2026 NDAA as established precedent, RECEIPTS Act (Ernst, $300M AI audit + legacy system replacement), FY2024 NDAA 1.5% cancellation trigger, five specific FY2027 NDAA provisions to push (Nunn-McCurdy second-breach reauthorization, right to repair expansion, SGE database, AUMF sunset amendment, USAID reconstitution); (3) federal agency pressure points including GAO investigation request mechanism (any member, email to CongRel@gao.gov, 6-18 month timeline), DoD IG Hotline process ((800) 424-9098, anonymous available), and FOIA pathways for DFAS, DCMA, and SAM.gov contract research; (4) ten organizations with specific roles and engagement mechanisms (POGO, National Priorities Project, Taxpayers for Common Sense, Stimson, Quincy, Center for International Policy, Win Without War, FCNL, AFSC, Council for a Livable World, Brennan Center, IAVA); (5) state-level leverage: state legislative AUMF resolutions (FCNL/Win Without War coordination networks), state pension fund shareholder engagement on contractor accountability (CalSTRS holdings in top contractors), National Guard deployment resistance grounded in December 2025 SCOTUS ruling blocking federalization of IL/TX Guard; (6) five time-bounded 2026 windows with specific named action mechanisms.*

---

## Summary Assessment

The domain-deepening corpus is substantially stronger than a typical policy research collection. The majority of files are publication-ready with minor additions. The files built on prior research threads (criminal justice, anti-corruption, digital government) are exceptional. The weakest files are not bad — they are merely adequate where others are excellent. There are three clear tiers.

**Broad pattern across the corpus**: Updated data is consistently the strongest dimension. International benchmarks and fiscal estimates are the most variable. Counterargument engagement is strong in about half the files and perfunctory in the other half. Actionable intelligence is strong where the domain has identifiable pressure points; thin where reform requires constitutional or legislative change with long time horizons.

**The single most important gap across domains**: Several files have excellent supply-side evidence (what the problem is, what reform looks like) but thin demand-side political economy analysis (who specifically blocks reform, what their veto mechanism is, what has shifted their position historically). For a proposal aiming at publication and use by organizers, this is the deepening that would add the most value.

---

## Publication-Readiness Tiers

### Tier 1 — Publish As-Is (16 files)

These files are ready for public release, citation in advocacy materials, and use as evidence foundations for the proposal. Minor additions would improve them but are not required.

1. criminal-justice-evidence.md
2. domain-02-campaign-finance.md
3. domain-03-anti-corruption.md
4. domain-04-digital-government.md
5. domain-05-fiscal-reform.md
6. domain-08-media-information.md
7. domain-09-federalism.md
8. domain-12-infrastructure.md
9. electoral-reform-evidence.md *(elevated Session 263: counterarguments added, actionable intelligence section added, state-by-state organizing targets, Fair Representation Act legislative vehicle)*
10. environment-climate-evidence.md *(elevated Session 264: full file read confirmed Tier 2+ quality; EU Green Deal vs. US rollback comparative trajectory added, IRA rollback fiscal cost estimates added, actionable intelligence section added)*
11. healthcare-education-evidence.md *(elevated Session 264: counterarguments section added, fiscal estimates completed with Stanford/UCSF BIR savings quantified, actionable intelligence section added)*
12. tax-policy-evidence.md *(elevated Session 265: duplication with fiscal-reform resolved through explicit scope differentiation, ETI literature added, "starve the beast" empirical record added, actionable intelligence section added)*
13. judicial-independence-evidence.md *(elevated Session 267: fiscal estimates for SCOTUS expansion/term limits/ethics IG added as Section 12; Canada JAC/JAAC and South Africa JSC fully developed as Section 13; counterargument on court reform as politicization — norm erosion objection, FDR miscalculation, arms race problem — with counter-counter and policy sequencing resolution added as Section 14)*
14. housing-evidence.md *(elevated Session 268: Vienna/Singapore expanded with specific outcome numbers, fiscal estimates for zoning reform/LIHTC/CLT, actionable intelligence section, 2025-2026 rollback documentation)*
15. economic-concentration-evidence.md *(elevated Session 269: Section 10 added — live enforcement proceedings with remedy-phase specificity, named legislative vehicles, state AG coalition pattern, organizations with pressure-point specificity, three time-bounded 2026 windows)*
16. national-security-evidence.md *(elevated Session 270: actionable intelligence gap filled — Sections AI-1 through AI-6 added: FY2027 NDAA markup window as primary vehicle, H.R. 6751 2001 AUMF sunset with procedural posture, H.R. 7555 Audit the Pentagon Act with penalty structure, enacted 2002 AUMF repeal as precedent, RECEIPTS Act, GAO request mechanism, DoD IG Hotline process, FOIA pathways for DFAS/DCMA/SAM.gov, ten organizations with specific engagement mechanisms, state-level leverage including SCOTUS-grounded National Guard sovereignty argument, five time-bounded 2026 windows)*

### Tier 2 — Adequate, Minor Gaps (7 files)

Solid evidentiary foundations. Missing one or two dimensions that would make them Tier 1, but usable as-is for most purposes. Deepening one weak dimension would elevate each.

9. immigration-evidence.md
11. labor-evidence.md
12. reparations-evidence.md
13. social-safety-net-evidence.md
14. rights-protection-evidence.md
15. data-privacy-evidence.md
16. domain-03-democratic-participation.md

### Tier 3 — Needs Targeted Deepening (0 files)

All five original Tier 3 files have been elevated. See Session 263–265 notes above.

---

## Per-Domain Quality Cards

Rubric dimensions: **Updated data** (current through 2024–2025) | **Int'l benchmarks** (specific peer-nation evidence with numbers) | **Counterarguments** (strongest objections engaged) | **Fiscal estimates** (cost/benefit with methodology) | **Sourcing** (named organizations, reports, years) | **Actionable intelligence** (near-term actions, organizations, pressure points)

---

### Domain 1: Electoral Reform
*File: electoral-reform-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | 2024 OECD turnout table, 2024 Alaska RCV repeal vote (50.1/49.9), 2025 bipartisanship study |
| Int'l benchmarks | Adequate | NZ MMP transition outcomes quantified; Ireland STV; IPU 32% vs 23% gender gap. Needs Germany/Scandinavia AVR operational detail |
| Counterarguments | Needs Work | RCV complexity objection addressed. Duverger's Law critique and "throwing away votes" framing not engaged. No treatment of incumbency-protection objections to PR |
| Fiscal estimates | Adequate | $200-400M RCV adoption estimate; $500M-1B PR transition estimate present but not sourced to specific methodology |
| Sourcing | Adequate | FairVote, IPU, Ballotpedia cited but several figures lack primary sources |
| Actionable intelligence | Needs Work | No state-by-state organizing targets, no specific campaigns to support, no legislative vehicles identified |

**Verdict**: ~~Tier 3~~ **Tier 1** *(elevated Session 263)*. Counterargument section added addressing voter confusion, strategic voting elimination, and incumbency protection objections to PR. Actionable intelligence section added with specific state campaigns (Maine, Alaska RCV defense), legislative vehicles (Fair Representation Act), and AVR organizing targets.

---

### Domain 2: Campaign Finance
*File: domain-02-campaign-finance.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | $15.9-16B 2024 total, $1.9B dark money record, Musk $277-300M — all 2024 figures with sourcing |
| Int'l benchmarks | Strong | Canada $1,750 cap + corporate ban; Germany €133M subsidy cap; UK Electoral Commission structure; France 1995 corporate ban — specific numbers throughout |
| Counterarguments | Strong | First Amendment/Buckley doctrine engaged; political speech argument addressed; "money equals speech" doctrinal chain traced and challenged |
| Fiscal estimates | Strong | DISCLOSE Act "essentially zero cost" characterization; 6:1 matching $1-2B/cycle estimate |
| Sourcing | Strong | Doctrinal chain from Buckley through McCutcheon cited precisely; FEC data; academic sourcing |
| Actionable intelligence | Strong | Seattle Democracy Vouchers as replicable model; specific federal legislative vehicles; small-dollar matching evidence with Connecticut CEP 76% data |

**Verdict**: Tier 1. One of the strongest files in the corpus. The only meaningful gap is that it could more explicitly name the specific organizations running current campaigns (End Citizens United, Issue One, etc.) for organizer use.

---

### Domain 3: Anti-Corruption
*File: domain-03-anti-corruption.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | 2024 CPI score (64/100, ranked 29th), 2025 tariff-trading pattern, January 2025 IG firings, DOGE NUMIDENT access — all with dates |
| Int'l benchmarks | Strong | Singapore CPIB (3rd globally), Hong Kong ICAC founding context, Denmark #1/#2 CPI, New Zealand, Australia NACC with specific comparison metrics |
| Counterarguments | Strong | Whistleblower chilling effect, "witch hunt" framing, institutional costs all addressed |
| Fiscal estimates | Strong | $300-500M independent agency; $14 return per $1 IG spending — both with sourcing methodology |
| Sourcing | Strong | 79+ citations; specific DOGE database access details with SSN counts, dollar amounts |
| Actionable intelligence | Strong | False Claims Act qui tam mechanism as near-term lever; OGE reform pathway; specific IG offices to defend |

**Verdict**: Tier 1. Exceptional. The DOGE access section (NUMIDENT 548M SSNs, Treasury $5.4T payments) is uniquely valuable as a 2025-specific evidence anchor. The revolving door section's natural experiment on patent examiners is the kind of causal evidence that survives hostile cross-examination.

---

### Domain 3b: Democratic Participation
*File: domain-03-democratic-participation.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | Pew trust 17% (December 2025); Irish assembly 2024 failure documented; Netherlands 2025 climate assembly |
| Int'l benchmarks | Strong | Belgium Ostbelgien permanent model; vTaiwan 2024-2025 AI governance; Decidim 400+ orgs, 50 countries |
| Counterarguments | Adequate | Main objections covered (elite capture, advisory theater, cost). Doesn't engage the strongest one: that deliberative bodies can be more easily captured by organized minorities than broad elections |
| Fiscal estimates | Adequate | Citizens' Assembly ~$35-50M per assembly; PB $50-100M/year pilot — rough but present |
| Sourcing | Adequate | Key claims cited but some figures in fiscal section lack primary sources |
| Actionable intelligence | Strong | Resistance-participation connection section is genuinely useful; specific PB city models named |

**Verdict**: Tier 2. The Irish assembly failure case is the most important piece of content here — it prevents the naive reading of deliberative democracy as automatically successful and gives organizers an honest caution about implementation requirements. That alone earns it Tier 2.

---

### Domain 4: Digital Government
*File: domain-04-digital-government.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | IRS Direct File killed November 2025; DOGE-SSA NUMIDENT incident; DoD 7 consecutive audit failures; USDS renamed |
| Int'l benchmarks | Strong | Estonia X-Road (929 institutions); UK GDS institutional turbulence post-2020; Denmark MitID; India Aadhaar exclusion crisis + Supreme Court ruling; Brazil cautionary |
| Counterarguments | Strong | Unresolved tensions section is substantive — engages vendor lock-in, security risks, and equity gaps honestly |
| Fiscal estimates | Strong | IRS IMF 60-year failure $15B overbudget; HealthCare.gov anatomy; reform cost estimates present |
| Sourcing | Strong | Specific database names, access incident details, oversight report citations |
| Actionable intelligence | Strong | 7 specific near-term actions named; Domain 4 as foundational for 5 other domains argument is the file's most valuable strategic contribution |

**Verdict**: Tier 1. The sequencing argument — that digital government reform is a prerequisite for fiscal reform, anti-corruption work, and social safety net delivery — is the kind of cross-domain strategic insight that makes a proposal more than a list of reforms.

---

### Domain 5: Fiscal Reform
*File: domain-05-fiscal-reform.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | CAMT actual revenue $572M vs. $35B projected; IRS $80B → $41.8B rescinded; Pillar Two NCTI ~12.6%; Norway 2024 exit deferral fix |
| Int'l benchmarks | Strong | Nordic tax-to-GDP context; BC carbon model; Sweden 27% emissions reduction; Norway wealth tax emigration with fix |
| Counterarguments | Strong | Saez/Zucman vs. Auten/Splinter methodological dispute stated plainly; Laffer curve ETI-based analysis; capital flight with Norway counter-evidence |
| Fiscal estimates | Strong | $600-900B annual revenue portfolio; wealth tax range $117-300B with methodological gap explained |
| Sourcing | Strong | PSZ, Auten/Splinter, CBO, GAO, ProPublica IRS data — all with specifics |
| Actionable intelligence | Adequate | 1986 Tax Reform Act lessons present; coalition-building strategies noted. Could name specific current legislative vehicles more precisely |

**Verdict**: Tier 1. The buy-borrow-die mechanics section and the CAMT actual-vs.-projected failure are uniquely valuable. The honest treatment of the Saez/Zucman methodological dispute — acknowledging the challenge while explaining why the directional finding survives — is exactly the right approach for a publication aiming to survive hostile scrutiny.

---

### Domain 6: Judicial Independence
*File: judicial-independence-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | 564 threats FY2025; 131 additional FY2026; specific incident documentation (McConnell 400 voicemails, 6 credible death threats); V-Dem 0.75→0.57 decline; WJP 112th on civil justice access |
| Int'l benchmarks | Adequate | UK NAO / Germany Bundesrechnungshof comparison for audit; Germany 2024 constitutional entrenchment referenced. Needs: Canada's advisory board model, South Africa JSC — both in gaps list but not fully developed |
| Counterarguments | Adequate | Consent decree defiance documented thoroughly; institutional attack vectors identified. Missing: the strongest argument that court reform proposals are themselves politicization |
| Fiscal estimates | Strong | Section 12 (added Session 267): per-seat SCOTUS expansion costs ($1-1.2M annual recurring + $2-4M one-time); term limits CRS "modest" characterization with actuarial framing; ethics IG $15-50M range with DOJ OIG comparator; full summary table |
| Sourcing | Strong | USMS threat data, V-Dem 2025 report, WJP 2024-2025, NYT judge survey — all primary sources with dates |
| Actionable intelligence | Adequate | Cross-reference to litigation tracker; consent decree defiance synthesis is operationally useful. Doesn't identify which jurisdictions have leverage points for state-level judicial reform |

**Verdict**: ~~Tier 2~~ **Tier 1** *(elevated Session 267)*. Section 12 fills fiscal estimates gap with per-option cost analysis and summary table. Section 13 fully develops Canada JAC/JAAC (35-year Ontario track record, 2016 Trudeau reforms, lay membership design rationale) and South Africa JSC (constitutional basis, 23-member composition, public hearings model, ANC bloc-voting critique). Section 14 develops the norm erosion objection in full (McConnell/Calabresi framing, no-limiting-principle problem), counter-counter (Garland blockade, ACB self-contradiction, Dobbs/Bruen empirical record, V-Dem quantification), and policy resolution (sequenced procedural-before-structural approach with 72% ethics support as entry point).

---

### Domain 7: Rights Protection
*File: rights-protection-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | DOGE-SSA NUMIDENT litigation through April 2026; IRS-ICE data sharing court ruling; Washington state 2,671,776 Nlets queries documented; ICE spyware April 2026 |
| Int'l benchmarks | Adequate | Germany Article 8 assembly rights structure; France Loi Avia Constitutional Council ruling. GDPR enforcement coverage is in data-privacy file, not this one. Needs EU comparison on anti-protest legislation |
| Counterarguments | Adequate | Main First Amendment tensions present; Cop City RICO indictment documented. Missing: the legitimate security interest in infrastructure protection that makes the 16-state pipeline legislation politically durable |
| Fiscal estimates | Needs Work | No cost estimates for surveillance infrastructure reform, Federal Data Protection Agency, or anti-SLAPP enforcement costs |
| Sourcing | Strong | Specific incident documentation throughout — FTC v. Kochava, Washington DOL query count, ACLU FOIA outcomes |
| Actionable intelligence | Adequate | Organizational landscape named (Democracy Forward, ACLU, AFSCME); specific litigation vehicles cited. Doesn't clearly identify which state campaigns are highest-leverage |

**Verdict**: Tier 2. The DOGE database access arc is the freshest and most actionable evidence. The anti-protest legislative wave section is important context that many readers will be underinformed on.

---

### Domain 8: Media and Information
*File: domain-08-media-information.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | EU DSA €120M fine against X December 2025; RSF 2025 ranking 57th; Frances Haugen documents with specific percentages |
| Int'l benchmarks | Strong | ARD/ZDF Rundfunkbeitrag €18.36/month + 2018 Federal Constitutional Court ruling; Nordic Presstödsnämnden eligibility criteria; Finland media literacy curriculum grade-by-grade |
| Counterarguments | Strong | State-run media objection; First Amendment/Moody analysis; Substack market solutions; filter bubble deepening all engaged |
| Fiscal estimates | Strong | $1.60/citizen US vs $130 Norway per-capita public media; Brookings $650M borrowing cost methodology |
| Sourcing | Strong | González-Bailón et al. Science 2023 citation with methodology; Haugen document names specific; Moody v. NetChoice citation |
| Actionable intelligence | Adequate | EU DSA enforcement as model; press vibrancy legislative vehicles noted. Could more specifically name state-level media ownership campaigns and journalism fund initiatives |

**Verdict**: Tier 1. The González-Bailón asymmetry finding — conservative "exclusive corner" with no liberal equivalent — is the most important piece of empirical evidence in the file, directly countering the "both sides" media bias framing.

---

### Domain 9: Federalism
*File: domain-09-federalism.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | NPVIC 209 electoral votes / 17 states + DC as of April 2026; Birmingham minimum wage preemption arc with Eleventh Circuit language; EPI preemption tracker |
| Int'l benchmarks | Strong | Switzerland Finanzausgleich 85% floor; Germany Länderfinanzausgleich Constitutional Court challenges; Spain asymmetric federalism success-to-failure trajectory; Canada notwithstanding clause usage |
| Counterarguments | Strong | Anti-commandeering doctrine; NFIB v. Sebelius analysis; consolidation equity dilution problem (Indianapolis Unigov racial representation) all present |
| Fiscal estimates | Adequate | NELP $1.5B/year wage loss with city data; preemption tracker cited but no cost of preemption reform |
| Sourcing | Strong | Hajnal et al. and Grimmer et al. both cited with methodological dispute noted; Billings et al. 2024 Journal of Public Economics peer-reviewed natural experiment |
| Actionable intelligence | Strong | NPVIC current status; NLC compact success factors; anti-commandeering as state resistance strategy |

**Verdict**: Tier 1. The preemption section — particularly NELP's wage loss quantification and the EPI tracker — is directly useful for state legislative organizing. The Colorado River cautionary design case is valuable for any cross-state compact design work.

---

### Domain 12: Infrastructure
*File: domain-12-infrastructure.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | IIJA cliff September 30, 2026 (time-sensitive); BEAD $42.45B authorized, zero distributed as of August 2025; ACP death 23M households; NEVI $879M rescission |
| Int'l benchmarks | Strong | South Korea broadband as utility; Germany Autobahn maintenance decline (cautionary); Netherlands water boards; Japan seismic resilience |
| Counterarguments | Adequate | IIJA funding debate; transit vs. highway allocation tensions noted. Doesn't engage the strongest fiscal objection: that IIJA borrowed against future spending and created maintenance obligations the federal government cannot sustain |
| Fiscal estimates | Strong | $568B allocated / $275B obligated / $293B vulnerable breakdown; $1 trillion deferred maintenance; FTA demand vs. appropriation gap ($45.1B vs. $3.8B) |
| Sourcing | Strong | ASCE 2025 Report Card; FERC Orders 1920/1920-A/B; specific rescission amounts with dates |
| Actionable intelligence | Strong | Time-sensitive state DOT actions; BEAD advocacy targets; water justice campaigns; grid/energy leverage points all specified |

**Verdict**: Tier 1. The IIJA cliff (September 30, 2026) is the single most time-critical piece of intelligence in the entire corpus — 5.5 months from now. Advocates who don't know this number are operating without critical information.

---

### Domain 13: Housing
*File: housing-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | 4.03 million gap (2025); 22.4 million cost-burdened renters (record, 2024); 770,000 HUD PIT Count 2024 (+18%); Q4 2024 homeownership data by race; 2025-2026 rollback documentation (AFFH suspension, HUD budget cuts, voucher under-funding) |
| Int'l benchmarks | Strong | Vienna expanded: €150-200k/unit construction cost, €7-8/sqm rent, 26,000 waitlist, €53,340 income ceiling, Klimabonus passive-house mandate. Singapore HDB added: 79% coverage, 88.9% homeownership rate, SGD 500-600k resale price, SGD 200-350k/unit construction cost, Land Acquisition Act as irreplaceable enabling condition |
| Counterarguments | Strong | Diamond et al. 2019 San Francisco rent control study (15% long-run supply reduction); Hsieh-Moretti correction with caveat; filtering evidence both directions |
| Fiscal estimates | Strong | Zoning reform: YIMBY Act near-zero cost, transportation conditioning $5-15B leverage, Hsieh-Moretti $1.7T GDP gain, Minneapolis 6-10% rent reduction. LIHTC: $400-600k/unit current vs. $280-420k direct grant alternative, Vienna non-profit analog $180-280k/unit. CLT scaling: $2.5-4B over 10 years vs. $31-37.5B voucher equivalent |
| Sourcing | Strong | Harvard JCHS, HUD AHAR, NLIHC, Glaeser/Gyourko NBER all primary-sourced; GAO, CBO, Lincoln Institute, Grounded Solutions Network added for fiscal section |
| Actionable intelligence | Strong | Section 13 added: YIMBY Act S.1614/H.R.3009, state pressure points (CA, CO, MT, NY, TX through 2026), organizations (NLIHC, Up For Growth, Grounded Solutions, LISC, Habitat), FY2027 appropriations and AFFH litigation pressure points, LIHTC Ways and Means window, 2026 state ballot initiatives |

**Verdict**: ~~Tier 2~~ **Tier 1** *(elevated Session 268)*. Vienna Gemeindebau section fully developed with specific outcome numbers. Singapore HDB added as Section 9.1b with full coverage, cost, and transferability analysis including the Land Acquisition Act constraint that is rarely stated plainly. Fiscal estimates subsection added under Section 12 covering zoning reform, LIHTC alternatives, and CLT scaling with comparison figures. Actionable intelligence added as Section 13. 2025-2026 rollback documentation added as Section 14 covering AFFH suspension, HUD budget/staff reductions, Section 8 under-funding, and the manufactured housing code update as the surviving regulatory gain.

---

### Domain 16: Immigration
*File: immigration-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | $164.65/day FY2025 ICE bed rate; One Big Beautiful Bill $45B detention expansion (July 2025); Penn Wharton mass deportation fiscal model (July 2025); Fortune 500 46% immigrant-founded (2025) |
| Int'l benchmarks | Adequate | Canada Express Entry, Germany Skilled Immigration Act, Australia mentioned in gaps list but not developed in sections read. Fiscal arithmetic is the file's real strength |
| Counterarguments | Adequate | Card-Borjas wage debate referenced; visa overstay data (40%+ legal entry) present as reframing tool. Enforcement-only objections not deeply engaged |
| Fiscal estimates | Strong | AIC $17,121 per deportation; $315B one-time mass deportation; Penn Wharton GDP -3.3% by 2034; CBO $1.3T GDP boost over 10 years — full fiscal picture |
| Sourcing | Strong | AIC, Penn Wharton, CBO, ITEP all cited with specific report dates and methodologies |
| Actionable intelligence | Adequate | ATD compliance rate evidence useful for advocates; fiscal arithmetic directly usable in public debate. Specific campaigns and legislative vehicles not identified |

**Verdict**: Tier 2. The Penn Wharton mass deportation fiscal model — GDP -3.3% by 2034, $987B fiscal cost, federal deficit increases — is the most powerful single piece of evidence in this file. The finding that mass deportation makes the average American worker poorer, not richer, directly refutes the dominant political framing.

---

### Domain 17: Labor
*File: labor-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | BLS 2024: private sector density 6.0%, public 32.5%; 2024 wage data; Cengiz et al. 2019 QJE — foundational recent |
| Int'l benchmarks | Adequate | Germany Works Councils and sectoral bargaining contrast; Manning LSE monopsony work; UK National Living Wage in gaps list but not developed in sections read |
| Counterarguments | Strong | Card-Krueger revolution traced including the methodological attacks and their resolution; Seattle UW hours-reduction finding mentioned in gaps but monopsony rebuttal is strong |
| Fiscal estimates | Needs Work | Explicit fiscal cost estimates for reform strands are in the gaps list as absent — this was acknowledged upfront. No numbers appear in sections read |
| Sourcing | Strong | Card and Krueger 1994 AER; Dube, Lester, Reich 2010; Cengiz et al. 2019 QJE; Nobel Prize citation — all primary sourced |
| Actionable intelligence | Needs Work | Sections read (120 of ~600+ lines) don't reach organizing/action section. What's visible is theoretical framework and evidence, not leverage points |

**Verdict**: Tier 2. The monopsony framework section is exceptional — the theoretical underpinning that explains why standard competitive-market predictions were wrong is exactly what's needed to give policymakers and advocates an answer to the "minimum wage destroys jobs" objection. Fiscal estimates gap acknowledged but not filled.

---

### Domain 18: Social Safety Net
*File: social-safety-net-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | 2021 CTC month-by-month poverty tracking; January 2022 reversal documented; TANF FY2023 19-state <10% cash assistance data |
| Int'l benchmarks | Strong | Canada Child Benefit 2016 reform outcomes; UK Child Benefit universal structure; Germany Kindergeld €250/month; Quebec $10/day program 10 years of follow-up data |
| Counterarguments | Strong | Labor supply elasticity — NIT experiments, Milligan/Stabile no-significant-labor-supply-response finding; TANF political economy of block grant diversion |
| Fiscal estimates | Adequate | Heckman 7-10% social rate of return; Quebec program pays for itself (Fortin); CTC costs present but full extension fiscal estimates not in sections read |
| Sourcing | Strong | Census Bureau CTC working paper; Columbia CPSP real-time tracking; Heckman NBER; Milligan/Stabile Journal of Labor Economics — all primary |
| Actionable intelligence | Adequate | CTC reinstatement as near-term vehicle; TANF reform advocacy identified. Could be stronger on specific organizations running current campaigns |

**Verdict**: Tier 2. The January 2022 reversal data — 3.7 million children falling into poverty in a single month when the CTC expansion expired — is among the most powerful pieces of evidence in the corpus. It proves the program worked with a precision that most social policy evidence cannot achieve.

---

### Domain 19: National Security
*File: national-security-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | Pentagon 8th consecutive audit failure December 2025 (26 material weaknesses, $4.65T assets/$4.7T liabilities); GAO projects failures through 2028; F-35 $2T lifetime cost (2024 GAO); AUMF 22-country expansion documented; 2002/1991 AUMFs repealed via FY2026 NDAA |
| Int'l benchmarks | Strong | G7 comparison table (UK NAO, Germany Bundesrechnungshof, France Cour des Comptes, Australia ANAO, Canada OAG) with structural differences and enforcement mechanisms |
| Counterarguments | Strong | Deterrence theory vs. accidental war risk treated; legitimate security argument for procurement continuity addressed; hollowing-out objection to audit requirements — classified program constraint vs. bookkeeping failure distinction — precisely engaged |
| Fiscal estimates | Adequate | F-35 $2T with trajectory; $4.25B equivalent of 0.5% Audit the Pentagon Act penalty; $60-70M contractor lobbying annual figure; fraud exposure gap acknowledged |
| Sourcing | Strong | GAO reports specific (GAO-25-108191, GAO-24-106703, GAO-25-107558, GAO-25-108500); POGO Fact Sheet; SIPRI 2024; bill numbers for all named legislation — all primary |
| Actionable intelligence | Strong | Sections AI-1 through AI-6: FY2027 NDAA as primary vehicle with specific provisions; H.R. 6751 (Jayapal-Massie 2001 AUMF sunset) and H.R. 7555 (Audit the Pentagon Act) with procedural posture; GAO request mechanism named; DoD IG Hotline ((800) 424-9098); FOIA pathways (DFAS, DCMA, SAM.gov); ten organizations with specific URLs and engagement mechanisms; state legislative resolutions, pension fund shareholder engagement, and National Guard sovereignty (December 2025 SCOTUS ruling) as state-level levers; five time-bounded 2026 windows |

**Verdict**: ~~Tier 3~~ ~~Tier 2+~~ **Tier 1** *(elevated Session 270)*. Counterargument section developed in Session 263. Actionable intelligence gap filled in Session 270: six subsections covering the FY2027 NDAA markup window (highest priority), specific named legislation with bill numbers and procedural posture, federal agency pressure points including the GAO investigation request mechanism (any member can request via email) and DoD IG Hotline, FOIA pathways for contracting records, ten organizations with specific engagement mechanisms beyond URLs, state-level leverage through AUMF resolutions / pension fund shareholder engagement / National Guard sovereignty argument, and five named 2026 windows with specific action mechanisms for each.

---

### Domain 21: Data Privacy
*File: data-privacy-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | FTC December 2024 Mobilewalla/Gravy Analytics actions; ICE spyware April 2026; 70+ lawmakers ICE data demand March 2026; PCLOB non-operational January 2025 |
| Int'l benchmarks | Strong | GDPR €5.88B total fines; EU AI Act February 2025 biometric enforcement; NIST FRVT error rates by demographic in precise numbers; RTB 294B daily exposures (US) documented |
| Counterarguments | Adequate | ADPPA failure anatomy (California preemption fight, Cantwell enforcement objection) detailed. Industry lobbying strategy documented. Missing: the genuine tension between data availability and beneficial uses (fraud prevention, medical research) |
| Fiscal estimates | Adequate | $277.97B data broker market; <50 FTC staff vs 280-person DPC comparison; Federal DPA staffing implications present but no full cost estimate |
| Sourcing | Strong | FTC enforcement actions by date and case name; Senate Commerce Committee December 2022 data pricing documentation; ICCL RTB report; specific Babel Street contract figures |
| Actionable intelligence | Adequate | State privacy law landscape; ADPPA successor APRA status noted; FTC leadership change implications. Specific organizing campaigns not identified |

**Verdict**: Tier 2. The RTB architecture section — explaining how a smartphone ad impression moves to a federal agency without a warrant in 100 milliseconds — is the clearest explanation of the surveillance pipeline in any document in this corpus. The $160 Planned Parenthood location data price point is a concrete harm anchor.

---

### Domain 22: Reparations
*File: reparations-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | 2022 Federal Reserve SCF: $284,310 white vs $44,100 Black median wealth (published October 2023); Evanston 44 recipients through early 2026; California 2024 14-bill package outcome |
| Int'l benchmarks | Adequate | South Africa TRC reparations failure (R30,000 vs US$375M recommendation) as cautionary case. Japanese internment and Rosewood in gaps list and partially developed. Missing: Germany's Holocaust reparations as the structural success model |
| Counterarguments | Adequate | COMPAS/Northpointe algorithmic bias; EEOC enforcement gap (88,531 charges, 111 lawsuits). Core objections to reparations themselves (targeting, amount determination, descendant definition) not fully engaged |
| Fiscal estimates | Adequate | Citigroup $16T GDP lost; McKinsey $1-1.5T/decade cost of racial inequality. No estimate of what HR 40 commission process would cost or what payment levels might be |
| Sourcing | Strong | Federal Reserve SCF (October 2023); Urban Institute; Richmond Fed (2022); Brandeis Heller School (2023) — primary sourced throughout |
| Actionable intelligence | Adequate | Evanston as replicable model; HR 40 current status; California 2024 package documented. Doesn't identify which jurisdictions are highest-leverage for local reparations ordinances |

**Verdict**: Tier 2. The mathematical demonstration that the percentage gains framing obscures widening absolute gaps — median Black wealth rose 61% but the absolute dollar gap grew by $50,000 in the same period — is the single most important methodological contribution in this file. Anyone arguing from percentages will be factually misrepresenting the data.

---

### Economic Concentration
*File: economic-concentration-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | RAND 2024 pharma pricing 2.78x; De Loecker 2020 QJE markups 1.18→1.67; Treasury 2022 labor market competition; 2024 hospital consolidation data |
| Int'l benchmarks | Strong | Philippon EU vs. US financial services fees (5% vs 8-9% of GDP); OECD broadband pricing table; EU digital market comparisons throughout |
| Counterarguments | Strong | Cepr VoxEU killer acquisitions digital-market critique; Cunningham-Ederer-Ma pharmaceutical evidence distinguished from digital; consumer welfare standard origin and critique |
| Fiscal estimates | Strong | $700-800B excess financial services fees/year; $30-50B excess broadband costs/year; $1.3-1.5T/year labor share transfer |
| Sourcing | Strong | De Loecker et al. 2020 QJE; Bork 1978 "The Antitrust Paradox"; Manning 2003 Princeton; Azar, Marinescu, Steinbaum 2022 RES — foundational academic sourcing |
| Actionable intelligence | Strong | Section 10 added: live enforcement proceedings with remedy-phase specificity; CALERA/AICOA/Open App Markets Act/Farm System Reform Act named; state AG coalitions (TX, NY, CA, IL) documented; Open Markets Institute, American Economic Liberties Project, EFF, Demand Progress, state PIRGs identified; three time-bounded 2026 windows (Google remedy record, FY2027 appropriations, state AG coalition expansion) |

**Verdict**: ~~Tier 2~~ **Tier 1** *(elevated Session 269)*. Section 10 fills the actionable intelligence gap: live FTC/DOJ cases named with their specific procedural posture and the public participation mechanisms available in each; legislative vehicles identified by bill name with committee history; state AG enforcement documented as the structurally resilient enforcement alternative when federal agencies are under-resourced; organizations listed with specific roles and URLs; three near-term 2026 windows identified with action mechanisms for each. The labor market concentration section — showing labor markets are more concentrated than product markets by HHI, with 36% of US workers in highly concentrated local labor markets — remains the most important institutional finding because it provides the theoretical bridge between antitrust and labor policy.

---

### Domain 20: Tax Policy
*File: tax-policy-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Adequate | ProPublica IRS data (2014-2018 period); TCJA post-reform (2018); OECD 2024 table present. Sections read don't include 2025-specific updates |
| Int'l benchmarks | Strong | Full OECD tax-to-GDP table (Denmark 45.2%, US 25.6%); Sweden pre-filled returns 74%; Estonia 98% digital filing; Pillar Two 2024 implementation status |
| Counterarguments | Adequate | Auten/Splinter methodological challenge to PSZ present; corporate tax incidence dispute noted. ETI literature and "starve the beast" empirical literature in gaps list but not yet in sections read |
| Fiscal estimates | Adequate | Billionaire effective rate 23% (Saez/Zucman); compliance cost $400B/year (NTUF 2024) |
| Sourcing | Strong | Saez/Zucman BPEA 2019; ProPublica IRS files; Auten/Splinter 2024; OECD Revenue Statistics 2024/2025 |
| Actionable intelligence | Needs Work | Sections read (120 of ~600+ lines) don't reach actionable section; no organizing targets or legislative vehicles identified |

**Verdict**: ~~Tier 3~~ **Tier 1** *(elevated Session 265)*. Scope differentiated from domain-05-fiscal-reform.md: tax-policy-evidence.md now explicitly handles income tax structure, compliance burden, individual and corporate incidence analysis, and administrative reform; fiscal-reform handles wealth tax, corporate tax reform architecture, and revenue portfolio. ETI literature added, "starve the beast" empirical record added, actionable intelligence section added. Citation confusion resolved.

---

### Environment/Climate
*File: environment-climate-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Strong | EPA: 40 civil DOJ referrals 2025 (vs. 186 Obama, 67 Biden); 28 completed enforcement actions; 1,155 scientists targeted; NOAA 550+ positions eliminated; weather balloon launches suspended |
| Int'l benchmarks | Needs Work | Sections read (80 lines) don't reach international benchmarks. BC carbon model is in fiscal-reform file; this file's own benchmarks not yet visible |
| Counterarguments | Needs Work | Not visible in sections read |
| Fiscal estimates | Needs Work | Not visible in sections read |
| Sourcing | Adequate | Enforcement comparison figures present; organizational sourcing partially visible |
| Actionable intelligence | Needs Work | Not yet visible in sections read |

**Verdict**: ~~Tier 3~~ **Tier 1** *(elevated Session 264)*. Full file read confirmed Tier 2+ underlying quality — the opening density held throughout. Added: EU Green Deal vs. US rollback comparative trajectory with specific targets and timelines, fiscal cost estimates for IRA rollback and enforcement gap, and actionable intelligence section naming litigation vehicles and key advocacy organizations.

---

### Healthcare/Education
*File: healthcare-education-evidence.md*

| Dimension | Score | Notes |
|---|---|---|
| Updated data | Adequate | OECD health spending 2024 table present; infant mortality data; multi-payer reform evidence |
| Int'l benchmarks | Strong | US $14,775/capita vs. OECD average $4,986; South Korea $4,000 = 27 cents per US dollar, +4.6 year life expectancy; Japan 1.7 vs. US 5.6 infant mortality |
| Counterarguments | Needs Work | Not visible in sections read |
| Fiscal estimates | Needs Work | Stanford/UCSF study referenced for multi-payer BIR savings but no specific figure visible in sections read |
| Sourcing | Adequate | OECD data visible but specific report citation not confirmed in 80-line read |
| Actionable intelligence | Needs Work | Not visible in sections read |

**Verdict**: ~~Tier 3~~ **Tier 1** *(elevated Session 264)*. Full file read confirmed strong underlying evidence base. Counterarguments section added covering the "rationing" objection, innovation incentive concerns, and administrative complexity of multi-payer transition. Fiscal estimates completed with Stanford/UCSF BIR savings quantified, full single-payer transition cost range added, and actionable intelligence section added with specific near-term campaigns and legislative vehicles.

---

## Deepening Queue

Priority order based on publication impact, time sensitivity, and gap severity:

**Priority 1 — Fix before any public release: COMPLETE (Sessions 263–265)**

All four Priority 1 items are done. Brief record:

1. **environment-climate-evidence.md** — elevated to Tier 1, Session 264. Full file read confirmed Tier 2+ underlying quality. Added: EU Green Deal vs. US rollback comparative trajectory, IRA rollback fiscal cost estimates, actionable intelligence section with litigation vehicles and advocacy organizations.

2. **electoral-reform-evidence.md** — elevated to Tier 1, Session 263. Added: counterargument section addressing voter confusion, strategic voting elimination, and incumbency protection objections to PR. Added: actionable intelligence section with Maine/Alaska RCV defense campaigns, Fair Representation Act as legislative vehicle, and AVR state-level organizing targets.

3. **national-security-evidence.md** — elevated to Tier 2+, Session 263; elevated to Tier 1, Session 270. Session 263 added counterargument section. Session 270 filled actionable intelligence gap: Sections AI-1 through AI-6 added (FY2027 NDAA as primary vehicle, H.R. 6751 AUMF sunset, H.R. 7555 Audit the Pentagon Act, enacted 2002 AUMF repeal precedent, RECEIPTS Act, GAO request mechanism, DoD IG Hotline, FOIA pathways, ten organizations, state leverage, five 2026 windows).

4. **tax-policy-evidence.md** — elevated to Tier 1, Session 265. Scope differentiated explicitly from domain-05-fiscal-reform.md (tax-policy handles income tax structure, compliance, and incidence; fiscal-reform handles wealth tax, corporate reform, and revenue portfolio). Added: ETI literature, "starve the beast" empirical record, counterarguments, and actionable intelligence section.

**Priority 2 — Would elevate the overall corpus:**

5. **judicial-independence-evidence.md**: COMPLETE (Session 267). Fiscal estimates added as Section 12 (SCOTUS expansion, term limits, ethics IG, lower court expansion — with summary table). Canada JAC/JAAC and South Africa JSC fully developed as Section 13. Counterargument on court reform as politicization — norm erosion objection, FDR miscalculation, arms race problem, counter-counter, and sequenced policy resolution — added as Section 14. File elevated to Tier 1.

6. **labor-evidence.md**: Add fiscal cost estimates for minimum wage increases, sectoral bargaining pilots, and portable benefits schemes. Add actionable intelligence section naming current organizing campaigns (Amazon Labor Union, Starbucks Workers United, state minimum wage ballot initiative calendar).

7. **healthcare-education-evidence.md**: Full read + add counterarguments section and fiscal estimates. The opening OECD evidence is strong; likely just needs the back half developed.

**Priority 3 — Meaningful improvements:**

8. **reparations-evidence.md**: Add Germany Holocaust reparations as the structural success model (the one case where the framework worked). Add cost estimate for HR 40 commission process. Develop the objection-to-reparations section more fully — the targeting/descendant-definition questions will be the primary political friction.

9. **housing-evidence.md**: COMPLETE (Session 268). Vienna Gemeindebau expanded with construction cost comparisons, rent levels, waitlist data, income ceiling, Klimabonus, and co-housing model. Singapore HDB added as Section 9.1b. Fiscal estimates added for zoning reform, LIHTC alternatives, and CLT scaling. Actionable intelligence added as Section 13. 2025-2026 rollback documentation added as Section 14. File elevated to Tier 1.

10. **economic-concentration-evidence.md**: COMPLETE (Session 269). Section 10 added: live enforcement proceedings with remedy-phase specificity (Google Search remedy, FTC v. Amazon, DOJ v. Apple, DOJ v. Google Ad Tech, FTC PBM investigation); legislative vehicles (CALERA, AICOA, Open App Markets Act, Farm System Reform Act, state modernization); state AG coalition pattern (TX, NY, CA, IL as named actors); organizations (Open Markets Institute, American Economic Liberties Project, EFF, Demand Progress, state PIRGs); three time-bounded 2026 windows. File elevated to Tier 1.

---

## Cross-Domain Observations

**Structural gap 1 — Political economy is the weak dimension across the corpus.**
Nearly every file has strong evidence on what the problem is and what reform looks like. Far fewer files systematically analyze who specifically blocks reform, what their institutional veto point is, and what has historically shifted their position. The anti-corruption file (revolving door natural experiment, PIN collapse), the campaign finance file (doctrinal chain), and the fiscal reform file (1986 Tax Reform Act coalition analysis) are the exceptions. The others need a "why this hasn't passed" section that is as rigorous as the "what the evidence shows" section.

**Structural gap 2 — Implementation failure modes are underrepresented.**
Several international benchmarks are presented as success cases without adequate treatment of the failure modes or adaptation requirements. The notable exceptions: domain-03-democratic-participation.md's Irish assembly failure case, domain-04-digital-government.md's India Aadhaar exclusion crisis, and domain-09-federalism.md's Colorado River cautionary design case. Every file citing a positive international benchmark should identify at least one jurisdiction where the same reform failed and why.

**Structural gap 3 — The 2025-2026 rollback is documented inconsistently across files.**
Some files (anti-corruption, digital government, infrastructure, judicial independence, rights protection) have excellent incident-level documentation of the current administration's specific actions. Others (housing, labor, economic concentration) treat the current period primarily through the lens of pre-existing structural problems without documenting the specific 2025-2026 changes that have accelerated them. For a proposal arguing that this is an urgent crisis requiring immediate response, the incident-level 2025-2026 documentation needs to be consistent across all domains.

**Structural gap 4 — The cross-domain synthesis is present only in the proposal, not in the companion files.**
The proposal's theory of change (Section 5.4's 13 reinforcing feedback loops) argues that piecemeal reform underperforms. But the companion files are largely siloed: each makes the case for its domain in isolation. The strongest exception is domain-04-digital-government.md, which explicitly identifies itself as foundational for five other domains. A brief cross-reference section in each file identifying which other domains it reinforces and which domains it depends on would make the corpus more useful as an integrated argument rather than a collection of separate evidence bases.

**Strongest files — common characteristics:**
The Tier 1 files share: (a) specific numbers throughout with methodology acknowledged, (b) contested findings engaged rather than elided, (c) at least one failure case that makes the success cases more credible, (d) incident-level 2025-2026 documentation, and (e) actionable pressure points. The template for deepening the Tier 2 and 3 files is visible in the Tier 1 files.

**Most under-utilized evidence across the corpus:**
The V-Dem 0.75→0.57 decline figure from judicial-independence-evidence.md is a headline number that belongs in the proposal's Part I and in multiple other files' executive overviews. No other number in the corpus so precisely quantifies the democratic backsliding claim that the entire proposal is built on. It should be pulled forward wherever the case for urgency is being made.
