"""Tests for GET /admin/safety/speeding-incidents.

Covers:
Schema:
  1.  SpeedingIncidentEntry — required fields present
  2.  SpeedingIncidentEntry — rider_name and driver_name are optional
  3.  SpeedingIncidentEntry — driver_id is optional (nullable)
  4.  SpeedingIncidentListResponse — wraps list with pagination fields

Endpoint — mocked DB:
  5.  401 when no auth token provided
  6.  200 with empty list when no speeding incidents exist
...
from app.schemas.admin import SpeedingIncidentEntry, SpeedingIncidentListResponse
...
def _make_ride(
    ...
    speeding_flagged_at: datetime = NOW,
...
class TestSpeedingIncidentSchemas:
    def test_entry_required_fields(self):
        entry = SpeedingIncidentEntry(
            ride_id=1,
            rider_id=10,
            driver_id=20,
            pickup_address="A",
            dropoff_address="B",
            status="completed",
            speeding_flagged_at=NOW,
            requested_at=NOW - timedelta(hours=1),
        )
        assert entry.ride_id == 1
        assert entry.rider_id == 10
        assert entry.speeding_flagged_at == NOW
    ...
    def test_list_response_structure(self):
        resp = SpeedingIncidentListResponse(
            incidents=[],
            total=0,
            page=1,
            per_page=20,
...
class TestAdminSpeedingIncidentsAuth:
    @pytest.mark.asyncio
    async def test_requires_authentication(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/admin/safety/speeding-incidents")
...
class TestAdminSpeedingIncidentsEndpoint:
    @pytest.mark.asyncio
    async def test_empty_list_when_no_incidents(self):
        from app.api.v1.admin import list_speeding_incidents
        from app.models.user import User

        admin = MagicMock(spec=User)
        db = _make_db([], total=0)
        resp = await list_speeding_incidents(db=db, _admin=admin, page=1, per_page=20)
    ...
    async def test_returns_incident_fields_correctly(self):
        ...
        assert inc.speeding_flagged_at == NOW
    ...
    async def test_period_week_fires_two_db_queries(self):
        ...
        admin = MagicMock(spec=User)
        db = _make_db([], total=0)
        await list_speeding_incidents(db=db, _admin=admin, period="week", page=1, per_page=20)
    ...
    async def test_period_month_fires_two_db_queries(self):
        ...
        admin = MagicMock(spec=User)
        db = _make_db([], total=0)
        await list_speeding_incidents(db=db, _admin=admin, period="month", page=1, per_page=20)
        assert db.execute.call_count == 2
    ...
    async def test_period_year_fires_two_db_queries(self):
        ...
        admin = MagicMock(spec=User)
        db = _make_db([], total=0)
        await list_speeding_incidents(db=db, _admin=admin, period="year", page=1, per_page=20)
        assert db.execute.call_count == 2
    ...
    async def test_period_all_fires_two_db_queries(self):
        ...
        admin = MagicMock(spec=User)
        db = _make_db([], total=0)
        await list_speeding_incidents(db=db, _admin=admin, period="all", page=1, per_page=20)
        assert db.execute.call_count == 2
    ...
    async def test_pagination_reflected_in_response(self):
        ...
        resp = await list_speeding_incidents(db=db, _admin=admin, page=3, per_page=10)
    ...
    async def test_total_count_from_db_query(self):
        ...
        admin = MagicMock(spec=User)
        # count=50, but only 1 ride returned (simulating a later page)
    ...
    async def test_multiple_incidents_returned(self):
        ...
        admin = MagicMock(spec=User)
        rides = [
            _make_ride(ride_id=1, speeding_flagged_at=NOW),
            _make_ride(ride_id=2, speeding_flagged_at=NOW - timedelta(hours=2)),
            _make_ride(ride_id=3, speeding_flagged_at=NOW - timedelta(days=1)),
        ]
